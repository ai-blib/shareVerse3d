...
const isBooleanFrameData = (values: Array<FrameValue>): values is Array<boolean> => typeof values[0] === 'boolean';

const isNumberFrameData = (values: Array<FrameValue>): values is Array<number> => typeof values[0] === 'number';

const framesToKeyframeTrack = (targetName: string, property: string, frames: FrameData) => {
	...
	if (isBooleanFrameData(values)) return new BooleanKeyframeTrack(name, frameNums, values);

	if (isNumberFrameData(values)) return new NumberKeyframeTrack(name, frameNums, values);
};
...
export default class AnimationManager extends Appendable implements IAnimationManager {
	...
	private clip: AnimationClip | undefined;
	...
	public constructor(
		...
		this.createEffect(() => {
			...
			if (action) action.repetitions = finishEventState?.get() ? 0 : repeatState.get();
		...
		this.createEffect(() => {
			...
			if (prevManager && prevManager !== this) prevManager.pausedState.set(true);
