...
import { hiddenAppendables, nonSerializedAppendables, appendableRoot } from "../core/collections";
import { isPoint } from "./isPoint";
import nonSerializedProperties from "./nonSerializedProperties";
const serialize = async (children, skipUUID) => {
    const dataParent = [];
    for (const child of children) {
        ...
        for (const [key, type] of Object.entries(schema)) {
            ...
            if (t === "string" &&
                ...
                value = relativePath(fileCurrent.webkitRelativePath, file.webkitRelativePath);
            }
            else if (t === "number")
                value = toFixed(key, value);
...
export default async (versionStamp, children = appendableRoot, skipUUID) => {
    if (children instanceof Appendable)
        return serialize([children], skipUUID);
    const childs = [];
    for (const child of children)
