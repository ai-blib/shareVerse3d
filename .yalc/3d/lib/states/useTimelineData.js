...
const [setTimelineData, getTimelineData] = store([
    undefined
]);
...
createEffect(() => {
    ...
    const timeline = getTimeline();
    if (!timelineData || !timeline || !getTimelineRecord())
        return;
    const timelineInstances = new WeakSet(Object.keys(timelineData).map((uuid) => uuidMap.get(uuid)));
    // @ts-ignore
    const handle0 = onEditorChanges((changes) => {
        ...
        for (const [instance, changedProperties] of changes) {
            if (!timelineInstances.has(instance))
                continue;
            const { uuid } = instance;
...
export const processKeyframe = (cb, skipRefresh) => {
    const [timelineData] = getTimelineData();
    const timeline = getTimeline();
    ...
    if (path.length === 1)
        ...
        cb(timelineData, path[0], path[1], frame);
    if (!skipRefresh)
        timeline.data = timelineData;
};
onTimelineClearKeyframe(() => processKeyframe((timelineData, uuid, property, frame) => unset(timelineData, [uuid, property, frame])));
//# sourceMappingURL=useTimelineData.js.map