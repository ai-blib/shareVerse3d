...
createEffect(() => {
    if (!getTimelinePaused()) {
        setCameraComputed(getCameraStack().at(-1) ?? getEditorCamera() ?? mainCamera);
        return;
    }
    setCameraComputed(getEditorCamera() ?? getCameraStack().at(-1) ?? mainCamera);
}, [getCameraStack, getEditorCamera, getTimelinePaused]);
//# sourceMappingURL=useCameraComputed.js.map