...
const AudioRow = ({ instance, frames }) => {
    const src = useSyncState(getPrivateValue(instance, "srcState").get);
    const duration = useSyncState(getPrivateValue(instance, "durationState").get);
    ...
    useLayoutEffect(() => {
        if (!duration || frameKeys.length >= 2 || !timeline)
            return;
        timeline.mergeData({
            [instance.uuid]: {
    ...
    useLayoutEffect(() => {
        ...
        const handle = getTimelineFrame((frame) => {
            waveSurfer.setCurrentTime(Math.max(frame - startFrame, 0) * FRAME2SEC);
        });
        return () => {
            handle.cancel();
        };
    ...
    return _jsx("div", { ref: ref, style: { width, left: startFrame * FRAME_WIDTH } });
};
export default memo(AudioRow, diffProps);
//# sourceMappingURL=AudioRow.js.map