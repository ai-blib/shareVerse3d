...
const Editor = () => {
    ...
    useLayoutEffect(() => {
        ...
        if (selectedSignal.value === "World" ||
            !selectionTarget ||
            selectionTarget instanceof Setup) {
            addSetupInputs(handle, pane, selectionTarget instanceof Setup ? selectionTarget : targetSetup);
            return () => {
        ...
        if (!multipleSelectionTargets.length) {
            const { schema, defaults, componentName } = unsafeGetValue(selectionTarget, "constructor");
            const [ownParams, ownRest] = splitObject(getParams(schema, defaults, selectionTarget), Object.keys(defaultsOwnKeysRecordMap.get(defaults) ?? {}));
            ...
            if (generalParams)
                addInputs(handle, pane, "general", selectionTarget, defaults, generalParams, true);
            ...
            if (transformParams0) {
                ...
                addInputs(handle, pane, "transform", selectionTarget, defaults, transformParams);
                innerTransformParams &&
                    addInputs(handle, pane, "inner transform", selectionTarget, defaults, innerTransformParams);
            }
            const [animationParams, animationRest] = splitObject(transformRest, ["animation", "animationPaused", "animationRepeat"]);
            animationParams &&
                addInputs(handle, pane, "animation", selectionTarget, defaults, animationParams);
            ...
            displayParams &&
                addInputs(handle, pane, "display", selectionTarget, defaults, displayParams);
            const [effectsParams, effectsRest] = splitObject(displayRest, [
                "bloom",
                "outline"
            ]);
            effectsParams &&
                addInputs(handle, pane, "effects", selectionTarget, defaults, effectsParams);
            const [adjustMaterialParams, adjustMaterialRest] = splitObject(effectsRest, [
                "metalnessFactor",
                "roughnessFactor",
                "opacityFactor",
                "envFactor",
            ...
            adjustMaterialParams &&
                addInputs(handle, pane, "adjust material", selectionTarget, defaults, adjustMaterialParams);
            ...
            materialParams &&
                addInputs(handle, pane, "material", selectionTarget, defaults, materialParams);
            ...
            pbrMaterialParams &&
                addInputs(handle, pane, "pbr material", selectionTarget, defaults, pbrMaterialParams);
            ...
            if (componentName === "dummy") {
                ...
                addInputs(handle, pane, componentName, selectionTarget, defaults, pbrMaterialRest, true).then((inputs) => inputs.stride.on("change", ({ value }) => {
                ...
                addInputs(handle, pane, componentName, selectionTarget, defaults, pbrMaterialRest, true);
        }
        return () => {
            setCameraFolder(undefined);
            handle.cancel();
    ...
    return (_jsxs("div", { className: "lingo3d-ui lingo3d-bg lingo3d-editor", style: {
            width: 300,
            height: "100%",
            display: "flex",
            flexDirection: "column"
        }, children: [_jsxs(AppBar, { selectedSignal: selectedSignal, children: [_jsx(CloseableTab, { children: "World" }), selectionTarget && (_jsx(CloseableTab, { selected: true, onClose: () => emitSelectionTarget(undefined), children: getComponentName(selectionTarget) }, selectionTarget.uuid))] }), _jsx("div", { style: {
                    flexGrow: 1,
                    overflowY: "scroll",
                    overflowX: "hidden",
                    paddingLeft: 8,
