...
createEffect(() => {
    if (getCameraRendered() !== mainCamera || getOrbitControls())
        return;
    const handle = getCameraDistance((cameraDistance) => (mainCameraManager.innerZ = cameraDistance));
    return () => {
        handle.cancel();
    };
}, [getCameraRendered, getOrbitControls]);
//# sourceMappingURL=mainCameraManager.js.map