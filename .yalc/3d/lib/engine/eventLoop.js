...
let paused = false;
const checkPaused = (val) => (paused = false);
...
export const timer = (time, repeat, cb) => {
    let count = 0;
...
const callbacksAnimation = new Set();
...
createEffect(() => {
    ...
    if (!renderer || (getFirstLoadBeforeRender() && !getFirstLoad()))
        return;
