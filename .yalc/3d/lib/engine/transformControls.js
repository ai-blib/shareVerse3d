...
import { emitTransformControls, onTransformControls } from "../events/onTransformControls";
...
export const sceneObj = scene;
export const lazyTransformControls = lazy(async () => {
...
createEffect(() => {
    ...
    const target = getSelectionNativeTarget() ??
    ...
    mode === "rotate" &&
        handle.watch(onTransformControls(() => {
            ...
            if ((rotation.x === -Math.PI && rotation.z === -Math.PI) ||
