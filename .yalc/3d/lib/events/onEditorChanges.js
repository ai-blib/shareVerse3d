...
import getChangedProperties, { saveProperties } from "../display/utils/getChangedProperties";
import { getEditorMounted } from "../states/useEditorMounted";
import { multipleSelectionTargetsFlushingPtr, getMultipleSelectionTargets, flushMultipleSelectionTargets } from "../states/useMultipleSelectionTargets";
...
export const [emitEditorChanges, onEditorChanges] = event();
createEffect(() => {
    if (!getEditorMounted())
        return;
    const instances = new Set();
    const getInstances = throttleTrailing(() => {
        if (multipleSelectionTargetsFlushingPtr[0])
            return;
        instances.clear();
    ...
    const handleFinish = () => flushMultipleSelectionTargets(() => emitEditorChanges(
    //todo: optimize array spread in the future
    // @ts-ignore
    [...instances].map((instance) => [instance, getChangedProperties(instance)])));
    // @ts-ignore
...
//# sourceMappingURL=onEditorChanges.js.map