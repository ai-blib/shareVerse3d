import { AmbientLight, AnimationClip, Bone, BufferGeometry, ClampToEdgeWrapping, Color, DirectionalLight, EquirectangularReflectionMapping, Euler, FileLoader, Float32BufferAttribute, Group, Line, LineBasicMaterial, Loader, LoaderUtils, MathUtils, Matrix3, Matrix4, Mesh, NumberKeyframeTrack, Object3D, OrthographicCamera, PerspectiveCamera, PointLight, PropertyBinding, Quaternion, QuaternionKeyframeTrack, RepeatWrapping, Skeleton, SkinnedMesh, SpotLight, Texture, TextureLoader, Uint16BufferAttribute, Vector3, Vector4, VectorKeyframeTrack, MeshStandardMaterial } from 'three';
...
class FBXTreeParser {
    ...
    parseParameters(materialNode, textureMap, ID) {
        ...
        else if (materialNode.DiffuseColor &&
        ...
        else if (materialNode.EmissiveColor &&
        ...
        connections.get(ID).children.forEach(function (child) {
            ...
            switch (type) {
                ...
                default:
                    console.warn('FBXLoader: %s map is not supported, skipping texture.', type);
                    break;
            }
        });
        return parameters;
    ...
    getTexture(textureMap, id) {
        ...
        if ('LayeredTexture' in fbxTree.Objects && id in fbxTree.Objects.LayeredTexture) {
            console.warn('FBXLoader: layered textures are not supported. Discarding all but first layer.');
            ...
            for (const nodeID in DeformerNodes) {
                ...
                if (deformerNode.attrType === 'Skin') {
                    ...
                    if (relationships.parents.length > 1)
                        console.warn('FBXLoader: skeleton attached to more than one geometry is not supported.');
                    ...
                    if (relationships.parents.length > 1)
                        console.warn('FBXLoader: morph target attached to more than one geometry is not supported.');
    ...
    // parse nodes in FBXTree.Objects.QuickDrawModal
...
class GeometryParser {
    ...
    parse(deformers) {
        ...
        if (this.negativeMaterialIndices === true) {
            console.warn('FBXLoader: The FBX file contains invalid (negative) material indices. The asset might not render as expected.');
        }
        return geometryMap;
    }
                            ...
                            if (layerCurveNodes[i] === undefined) {
                                ...
                                if (modelID !== undefined) {
                                    ...
                                    if (rawModel === undefined) {
                                        console.warn('FBXLoader: Encountered a unused curve.', child);
                                        return;
                                    }
                                    const node = {
