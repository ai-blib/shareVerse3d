...
const objectURLExtensionMap = new Map();
const fileObjectURLMap = new Map();
export const objectURLFileMap = new Map();
export const getExtensionIncludingObjectURL = (src) => objectURLExtensionMap.get(src) ?? splitFileName(src)[1]?.toLowerCase();
export const createObjectURL = (file, extension = splitFileName(file.webkitRelativePath)[1]?.toLowerCase()) => forceGet(fileObjectURLMap, file, () => {
