...
export default function () {
    if (this.done)
        return;
    this.createEffect(() => {
        if (getCameraRendered() !== this.camera ||
            !this.mouseControlState.get())
            return;
        if (getCameraPointerLock() === this.camera) {
            const handleMove = (e) => this.gyrate(e.movementX, e.movementY);
        ...
        const handleMove = (e) => {
            ...
            started &&
                this.gyrate((movementX / window.innerWidth) * 3000, (movementY / window.innerHeight) * 3000);
        };
        if (isMobile) {
            ...
            const handleTouchEnd = (e) => {
                if (identifier === undefined)
                    return;
                if (e.changedTouches[e.changedTouches.length - 1].identifier ===
                    identifier) {
    ...
    this.createEffect(() => {
        ...
        if (this.mouseControlState.get() !== true ||
            camera !== this.camera ||
            !getEditorPlay())
            return;
...
//# sourceMappingURL=enableMouseControl.js.map