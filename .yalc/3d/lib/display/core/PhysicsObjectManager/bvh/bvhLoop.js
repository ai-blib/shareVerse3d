...
import { emitFallEvent, listenerInitEvent, removeAll } from "../../utils/bvhEvent";
...
createEffect(function () {
    if (!getEditorPlay() || !getFirstLoad() || getBVHComputing())
        return;
    // @ts-ignore
    window.boxBounds = {
        min: {
        ...
        for (const characterManager of bvhCharacterSet) {
            ...
            if (centripetal) {
                ...
                for (const boundsTree of bvhArray) {
                    ...
                    calBoundary(
                    // @ts-ignore
                    window.boxBounds, boundsTree.geometry.boundingBox);
                    boundsTree.shapecast({
                        ...
                        intersectsTriangle: (tri) => {
                            ...
                            if (distance < capsuleRadius) {
                                ...
                                if (Math.abs(triPoint.y).toFixed(3) ===
