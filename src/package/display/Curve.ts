...
const createFor = <Result, Data>(dataList: Array<Data>, create: (data: Data, cleanup: Cancellable) => Result) => {
...
export default class Curve extends Appendable implements ICurve {
	...
	public constructor() {
		...
		this.createEffect(() => {
			...
			if (this._points.length < 2) for (let i = 0; i < segments; ++i) bufferAttribute.setXYZ(i, 0, 0, 0);
			else {
		...
		this.createEffect(() => {
			const helpers = createFor(
				this.helperState.get() && getCameraRendered() === mainCamera ? this._points : [],
				(pt, cleanup) => {
