...
class Model extends Loaded<Group> implements IModel {
	...
	public async loadAnimation(url: string, name = url) {
		...
		const animation = (this.animations[name] = this.watch(
			new AnimationManager(name, clip, await this.loaded, repeatState, onFinishState, finishEventState)
		));
	...
	protected async load(url: string) {
		...
		const module =
			extension === 'fbx' ? await import('./utils/loaders/loadFBX') : await import('./utils/loaders/loadGLTF');
	...
	protected resolveLoaded(loadedObject3d: Group, src: string) {
		...
		for (const clip of loadedObject3d.animations) {
			...
			const animation = (this.animations[clip.name] = this.watch(
				new AnimationManager(clip.name, clip, loadedObject3d, repeatState, onFinishState, finishEventState)
			));
			if (this.playAllAnimation) {
	...
	public override findAll(name?: string | RegExp | ((name: string) => boolean)): Array<FoundManager> {
