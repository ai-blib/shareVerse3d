...
import IAnimatedObjectManager, { Animation, AnimationValue } from '../../../interface/IAnimatedObjectManager';
...
const animationValueToData = (val: AnimationValue) => {
	...
	let maxLength = 0;
	for (const [, { length }] of entries) length > maxLength && (maxLength = length);
	...
	implements IAnimatedObjectManager
{
	...
	public lazyStates() {
		...
		const { managerState, pausedState } = (this.states = {
			...
			finishEventState: new Reactive<EventFunctions | undefined>(undefined)
		});
		this.createEffect(() => {
