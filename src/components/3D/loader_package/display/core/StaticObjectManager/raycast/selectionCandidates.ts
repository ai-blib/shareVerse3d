...
export const addSelectionHelper = (helper: VisibleObjectManager, manager: Appendable) => {
	manager.outerObject3d.add(helper.outerObject3d);
	additionalSelectionCandidates.add(helper.nativeObject3d);

	const handle = onSelectionTarget(({ target }) => target === helper && emitSelectionTarget(manager));
...
const traverse = (
	targets: Array<Appendable | StaticObjectManager> | Set<Appendable | StaticObjectManager>,
...
export const getSelectionCandidates = throttleTrailing(
	(targets: Array<Appendable> | Set<Appendable> = appendableRoot) => {
		...
		if (overrideSelectionCandidates.size) {
			for (const candidate of overrideSelectionCandidates) selectionCandidates.add(candidate);
			return;
		}
		const [frozenSet] = getSelectionFrozen();
		traverse(targets, frozenSet);
		for (const candidate of additionalSelectionCandidates) selectionCandidates.add(candidate);
	}
);
