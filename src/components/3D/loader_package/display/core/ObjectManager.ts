...
{
	public constructor(public object3d = new Object3D() as T, unmounted?: boolean) {
		super(object3d);
	...
	public findAll(name?: string | RegExp | ((name: string) => boolean)): Array<FoundManager> {
		...
		if (name === undefined)
			...
			this.outerObject3d.traverse(child => {
				child.name === name && result.push(setManager(child, new FoundManager(child)));
			});
		else if (typeof name === 'function')
			this.outerObject3d.traverse(child => {
				name(child.name) && result.push(setManager(child, new FoundManager(child)));
			});
		else
			this.outerObject3d.traverse(child => {
				name.test(child.name) && result.push(setManager(child, new FoundManager(child)));
			});
		return result;
	}
}
