...
import getChangedProperties, { ChangedProperties, saveProperties } from '../display/utils/getChangedProperties';
...
createEffect(() => {
	...
	const getInstances = throttleTrailing(() => {
		...
		instances.clear();
		for (const target of getMultipleSelectionTargets()) instances.add(target);
	...
	const handleFinish = () =>
		flushMultipleSelectionTargets(() =>
			emitEditorChanges(
				...
				[...instances].map(instance => <const>[instance, getChangedProperties(instance)])
			)
		);
